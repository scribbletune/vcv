...
std::string netraRagaNames[] = {"SENAVATI", "HANUMATODI", "DHENUKA", "NATAKAPRIYA", "KOKILAPRIYA", "RUPAVATI"};
std::string getNetraRagaName(int idx)
{
  return netraRagaNames[idx];
}

struct RagaObj Senavati = {7, {0, 1, 3, 5, 7, 8, 9}};
struct RagaObj Hanumatodi = {7, {0, 1, 3, 5, 7, 8, 10}};
struct RagaObj Dhenuka = {7, {0, 1, 3, 5, 7, 8, 11}};
struct RagaObj Natakapriya = {7, {0, 1, 3, 5, 7, 9, 10}};
struct RagaObj Kokilapriya = {7, {0, 1, 3, 5, 7, 9, 11}};
struct RagaObj Rupavati = {7, {0, 1, 3, 5, 7, 10, 11}};

RagaObj getNetraRagaByIdx(int idx)
{
  switch (idx)
  {
  case 0:
...
struct NetraQuantizer : Module
{
  ...
  std::string currentRagaName = "SENAVATI";
  ...
  void process(const ProcessArgs &args) override
  {
    ...
    currentRagaName = getNetraRagaName(int(raga));
    ...
    RagaObj r = getNetraRagaByIdx(raga);
    int *p;
    p = r.indices;
...
struct NetraQuantizerWidget : ModuleWidget
{
  RagaNameTextFieldWidget *ragaNameText;
  RootNoteNameTextFieldWidget *rootNoteNameText;
  NetraQuantizerWidget(NetraQuantizer *module)
  {
    setModule(module);
    setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/Netra.svg")));
    ...
    RoundBlackKnob *ragaKnob = createParamCentered<RoundBlackKnob>(mm2px(Vec(10, 28)), module, NetraQuantizer::RAGA_PARAM);
    ragaKnob->snap = true;
    addParam(ragaKnob);

    RoundBlackKnob *rootNoteKnob = createParamCentered<RoundBlackKnob>(mm2px(Vec(10, 50)), module, NetraQuantizer::ROOT_NOTE_PARAM);
    rootNoteKnob->snap = true;
    addParam(rootNoteKnob);

    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(10.24, 77.478)), module, NetraQuantizer::CV_INPUT));

    addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(10.24, 108.713)), module, NetraQuantizer::CV_OUTPUT));
...
Model *modelNetraQuantizer = createModel<NetraQuantizer, NetraQuantizerWidget>("Netra");