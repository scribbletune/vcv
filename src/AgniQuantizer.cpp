...
std::string agniRagaNames[] = {"GAYAKAPRIYA", "VAKULABHARANAM", "MAYAMALAVAGOWLA", "CHAKRAVAKAM", "SURYAKANTAM", "HATAKAMBARI"};
std::string getAgniRagaName(int idx)
{
  return agniRagaNames[idx];
}

struct RagaObj Gayakapriya = {7, {0, 1, 3, 5, 7, 8, 9}};
struct RagaObj Vakulabharanam = {7, {0, 1, 3, 5, 7, 8, 10}};
struct RagaObj Mayamalavagowla = {7, {0, 1, 3, 5, 7, 8, 11}};
struct RagaObj Chakravakam = {7, {0, 1, 3, 5, 7, 9, 10}};
struct RagaObj Suryakantam = {7, {0, 1, 3, 5, 7, 9, 11}};
struct RagaObj Hatakambari = {7, {0, 1, 3, 5, 7, 10, 11}};

RagaObj getAgniRagaByIdx(int idx)
{
  switch (idx)
  {
  case 0:
...
struct AgniQuantizer : Module
{
  ...
  std::string currentRagaName = "GAYAKAPRIYA";
  ...
  void process(const ProcessArgs &args) override
  {
    ...
    currentRagaName = getAgniRagaName(int(raga));
    ...
    RagaObj r = getAgniRagaByIdx(raga);
    int *p;
    p = r.indices;
...
struct AgniQuantizerWidget : ModuleWidget
{
  RagaNameTextFieldWidget *ragaNameText;
  RootNoteNameTextFieldWidget *rootNoteNameText;
  AgniQuantizerWidget(AgniQuantizer *module)
  {
    setModule(module);
    setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/Agni.svg")));
    ...
    RoundBlackKnob *ragaKnob = createParamCentered<RoundBlackKnob>(mm2px(Vec(10, 28)), module, AgniQuantizer::RAGA_PARAM);
    ragaKnob->snap = true;
    addParam(ragaKnob);

    RoundBlackKnob *rootNoteKnob = createParamCentered<RoundBlackKnob>(mm2px(Vec(10, 50)), module, AgniQuantizer::ROOT_NOTE_PARAM);
    rootNoteKnob->snap = true;
    addParam(rootNoteKnob);

    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(10.24, 77.478)), module, AgniQuantizer::CV_INPUT));

    addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(10.24, 108.713)), module, AgniQuantizer::CV_OUTPUT));
...
Model *modelAgniQuantizer = createModel<AgniQuantizer, AgniQuantizerWidget>("Agni");